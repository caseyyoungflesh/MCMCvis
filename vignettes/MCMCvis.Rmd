...
Sample quantiles can now be specified directly using the `probs` argument in MCMCsummary, removing the need to define custom quantiles with the `func` argument. The default behavior is to provide 2.5%, 50%, and 97.5% quantiles. These probabilities can be changed by supplying a numeric vector or scalar to the `probs` argument. 
...
Setting `HPD = TRUE` will cause MCMCsummary to use `HPDinterval` from the `coda` package to compute highest posterior density intervals specific to the probability specified in the `probs` argument. In this case, `probs` should be a scalar. If it is not, MCMCsummary will use the first value in the `probs` vector. Note that for each parameter `HPDinterval` normally returns one interval chain. For convenience, the chains are pooled in MCMCsummary such that HPD intervals is computed across all posterior samples.
...
Plots can be scaled to see both the posterior and the prior distribution using the `post_zm` argument.

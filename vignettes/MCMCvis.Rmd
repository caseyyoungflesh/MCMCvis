...
`MCMCsummary` is used to output summary information from MCMC output. This function accepts `stanfit` objects (created with the `rstan` package), `mcmc.list` objects (create with the `rjags` or `coda` packages), `R2jags` output (created with the `R2jags` package), and matrices of MCMC output (one chain per column). The function automatically detects the object type and proceeds accordingly. Two decimal places are reported by default. This can be changed using the `digits` argument.
...
Individual parameters can also be specified. For example, one `alpha` (of many) may be specified. In this case, the square brackets should be ignored to only specify the `alpha[1]` parameter. Use the argument `ISB = FALSE` to specify particular parameters that contain brackets. ISB is short for 'Ignore Square Brackets'. 
...
Setting the `Rhat` and `n.eff` arguments to `FALSE` can be used to avoid calculating the Rhat statistic and number of effective samples, respectively. This can greatly increase the speed of the function with very large MCMC objects.
...
Just trace plot can be plotted with `type = 'trace'`. Just density plots can be plotted with `type = 'density'`. Default is `type = 'both'` which outputs both trace and density plots. Individual chains for the density plot can be output using the `ind` argument. Output is written to PDF by default to enable mroe efficient review of posteriors - this also reduces computation time. PDF output is particularly recommended for large numbers of parameters.
...
The orientation of the plot can also be change using the `horiz` argument. `ylab` is then used to specify an alternaive label on the 'estimate axis'.

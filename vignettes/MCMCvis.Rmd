...
Sample quantiles can now be specified directly using the `probs` argument in MCMCsummary, removing the need to define custom quantiles with the `func` argument. The default behavior is to provide 2.5%, 50%, and 97.5% quantiles. These probabilities can be changed by supplying a numeric vector or a scalar to the `probs` argument. 
...
Setting `HPD = TRUE` will cause MCMCsummary to use `HPDinterval` from the `coda` package to compute highest posterior density intervals specific to the probability specified in the `probs` argument. In this case, `probs` should be a scalar. If it is not, MCMCsummary will simply use the first value in the `probs` vector. Note that for each parameter `HPDinterval` returns one HPD interval for each chain. However, in MCMCsummary the chains are pooled such that the interval is computed across all samples for convenience.

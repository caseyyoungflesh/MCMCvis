...
`MCMCsummary` is used to output summary information from MCMC output. All digits are reported by default, though the number of significant digits displayed can be specified with `digits` (except for Rhat which is always rounded to 2 decimal places). We'll use the build in `mcmc.list` object data for the examples below, but model output of any of the supported types will behave in the same way.
...
Specific parameters can be specified to subset summary information. Square brackets in parameter names are ignored by default. For instance, all `alpha` parameters can be plotted using `params = 'alpha'`. As an alternative to significant digits, the number of decimal places to round output to can be specified with `round`.
...
Custom functions can be specified as well. If the output length of the specified function is greater than 1, an extra dimension is added to the function output. For instance, a vector becomes a matrix, a matrix a three dimensional array, and so forth.

...
- `potrace` - create trace plots of MCMC chains for particular parameters of interest
...
Just trace plot can be plotted with `type = 'trace'`. Just density plots can be plotted with `type = 'density'`. Default is `type = 'both'` which outputes both trace and density plots. Individual chains for the density plot can be output using the `ind` argument.
...
`ref_vis = FALSE` can be used to disable this feature. All median dots will be represented as 'closed' black circles. A vertical reference at 0 is plotted by default. The position of this reference line can be removed with the `ref_line` argument. `ref_line = NULL` removes the reference line altogether.

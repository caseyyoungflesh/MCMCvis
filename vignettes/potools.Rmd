...
`poplot` is used to plot density strips for MCMC output. Black ticks are used to specify posterior mean or median (depending on input for `centrality` argument), while grey ticks specify credible intervals (95% CI by default).
...
A vertical guideline at 0 is plotted by default, but can be removed with `g_lines = NULL`. Multiple guidelines can be plotted by concatenating argument input.

Graphical parameters for x-axis limitation, axis labels, title, density bar height, and density bar tick height and width can also be specified.
